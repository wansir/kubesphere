...
func ListNamespacedApplication(req *restful.Request, resp *restful.Response) {
	...
	result, err := openpitrix.ListApplication(runtimeId, conditions, limit, offset)
...
func DescribeApplication(req *restful.Request, resp *restful.Response) {
	...
	namespaceName := req.PathParameter("namespace")
	app, err := openpitrix.DescribeApplication(clusterId)
	...
	if runtimeId != app.RuntimeId {
		err = fmt.Errorf("rumtime not match %s,%s", app.RuntimeId, runtimeId)
		klog.V(4).Info(err)
...
func CreateApplication(req *restful.Request, resp *restful.Response) {
	...
	var app opmodels.OpenpitrixCreateClusterRequest
	...
	err = openpitrix.CreateApplication(namespace, app)
